...
- (id)init {
	self = [super initWithFrame:CGRectMake(0, 0, 0, 20.f)];
	...
	if (self) {
		...
		_containerContainerView = [[UIView alloc] initWithFrame:self.frame];
		...
		_containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 0, self.frame.size.height)];
		...
		_label = [[UILabel alloc] initWithFrame:CGRectMake(iconImageView.frame.size.width + 4.f, 0, 0, self.frame.size.height)];
...
- (void)setString:(NSString *)string {
	if (string) {
		...
		labelFrame.size.width = [_label.text sizeWithFont:_label.font constrainedToSize:self.frame.size lineBreakMode:UILineBreakModeTailTruncation].width;
